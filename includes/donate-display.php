<?php 
use \stripecustom\Service\StripePayment;
use \Stripe\Exception\CardException;
include_once(__DIR__.'/../stripe/stripe-payment.php');
...
global $wpdb;
global $post;
add_action('template_redirect', 'wp_donate_payment_proccess');
function wp_donate_payment_proccess(){
	global $wpdb;
	global $wp;
	global $post;
	$current_url = get_permalink($post->ID);
	$mysetting = $wpdb->get_results( "SELECT * FROM ".$wpdb->prefix."donate_setting" );
	if(isset($mysetting[0])){
		define("STRIPE_PUBLIC_KEY",$mysetting[0]->public_key); // set public key
		define("STRIPE_SECRET_KEY",$mysetting[0]->secret_key); // set secret key
	} else {
	...
	if(isset($_REQUEST['action'])){
		...
		if($_REQUEST['action']=='submitdonate'){		
			...
			$organization = $wpdb->_real_escape($_REQUEST['organization']);
			$address = $wpdb->_real_escape($_REQUEST['address']);
			$city = $wpdb->_real_escape($_REQUEST['city']);
			$country = $wpdb->_real_escape($_REQUEST['country']);
			...
			$donation_type = 'single payment';
			$r_frequency = 0;
			$r_times = 0;
			$amount = $wpdb->_real_escape($_REQUEST['amount']);
			$card_type = $wpdb->_real_escape($_REQUEST['card_type']);
			$x_card_num = $wpdb->_real_escape($_REQUEST['x_card_num']);
			$exp_month = $wpdb->_real_escape($_REQUEST['exp_month']);
			$exp_year = $wpdb->_real_escape($_REQUEST['exp_year']);
			$x_card_code = $wpdb->_real_escape($_REQUEST['x_card_code']);
			$comment = $wpdb->_real_escape($_REQUEST['comment']);
			...
			$stripe_response = $stripe_payment->chargeAmountFromCard($_POST);
			if (is_array($stripe_response) && isset($stripe_response['amount_refunded']) && $stripe_response['amount_refunded'] == 0 && isset($stripe_response['amount_captured']) && $stripe_response['amount_captured'] > 0) {		
				$q_status = "1";
				$wpdb->query(
						$wpdb->prepare("
				...
				$_SESSION['donate_msg'] = $stripe_response;
				header("location:".$current_url);
				exit;
			}
...
function wp_donate_form() {
    ob_start();
        global $wpdb;
    ?>
    	<form method="post" name="donate_form" id="donate_form" action="<?php echo get_permalink(get_the_ID()); ?>" autocomplete="off">
			<input type="hidden" name="action" value="submitdonate" />
			<table width="100%" cellspacing="4" cellpadding="4">
				<tr><td style="color:red;" colspan="4"><?php echo (isset($_SESSION['donate_msg']))?$_SESSION['donate_msg']:'';?></td></tr>
				<?php 
					if(isset($_SESSION['donate_msg'])){
						unset($_SESSION['donate_msg']);
					}
				?>
				...
				<tr>
					<td colspan="4" class="heading"><b>Donor information</b></td>
				</tr>					
				<tr>			
					<td class="title_cell" colspan="2">First name<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="first_name" id="first_name" value="" size="25" />
					</td>
				</tr>
				<tr>			
					<td class="title_cell" colspan="2">Last name<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" colspan="2" class="inputbox" name="last_name" value="" size="25" />
					</td>
				</tr>
				<tr>			
					<td class="title_cell" colspan="2">Organization</td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="organization" value="" size="30" />
					</td>
				</tr>
				<tr>			
					<td class="title_cell" colspan="2">Address<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="address" value="" size="50" />
					</td>
				</tr>	
				<tr>			
					<td class="title_cell" colspan="2">City<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="city" value="" size="15" />
					</td>
				</tr>		
				<tr>			
					<td class="title_cell" colspan="2">Country<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
				...
				<tr>			
					<td class="title_cell" colspan="2">State<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
				...
				<tr>			
					<td class="title_cell" colspan="2">Zip<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="zip" value="" size="15" />
					</td>
				</tr>
				<tr>			
					<td class="title_cell" colspan="2">Phone<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="phone" value="" size="15" />
					</td>
				</tr>																					
				<tr>			
					<td class="title_cell" colspan="2">Email<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" class="inputbox" name="email" id="email" value="" size="40" />
					</td>
				</tr>
				<tr>
					<td colspan="4" class="heading"><strong>Donation Information</strong></td>			
				</tr>
				
				
				<tr>
					<td class="title_cell" colspan="2" valign="top">Amount</td>				
					<td id="amount_container" colspan="2">
						$ <input type="text" class="inputbox" name="amount" id="amount" value="" size="10" />
					</td>
				</tr>		
				<tr id="tr_card_type">
					<td class="title_cell" colspan="2">Card type<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
				...
				<tr id="tr_card_number" >
					<td class="title_cell" colspan="2">Credit Card Number<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input type="text" id="x_card_num" name="x_card_num" class="inputbox" onkeyup="checkNumber(this)" value="" size="20" />
					</td>
				</tr>
				<tr id="tr_exp_date" >
					<td class="title_cell" colspan="2">Expiration Date<span class="required">*</span>
					</td>
					<td class="field_cell" colspan="2">					
						<select id="exp_month" name="exp_month" class="inputbox exp_month" >
				...
				<tr id="tr_cvv_code" >
					<td class="title_cell" colspan="2">Card (CVV) Code<span class="required">*</span></td>
					<td class="field_cell" colspan="2">
						<input id="x_card_code" type="text" name="x_card_code" class="inputbox" onKeyUp="checkNumber(this)" value="" size="20" />
					</td>
				</tr>
								
				
				<tr>			
					<td class="title_cell" colspan="2">Comment</td>
					<td class="field_cell" colspan="2">
				...
				<tr>
					<td colspan="4" align="left">
						<input type="button" class="button donate_btn_submit" name="btnSubmit" value="Donate Now" onClick="stripePay(event);">
						<div id="loader">
							<img alt="loader" src="<?php echo WP_DONATE_PATH; ?>/images/LoaderIcon.gif">
						</div>
					</td>
				</tr>										
			</table>
			...
			<script type="text/javascript">
				...
				function checkData() {
					...
					if (!amountValid) {
						var msg = "Please choose from pre-defined amounts or enter your own amount in the textbox";
						alert(msg);
						return;	
					}			
				...
				<script>
					function cardValidation () {
						...
						var cardNumber = jQuery('#x_card_num').val();
						var month = jQuery('#exp_month').val();
						var year = jQuery('#exp_year').val();
						var cvc = jQuery('#x_card_code').val();
					...
					function stripeResponseHandler(status, response) {
						if (response.error) {
							...
							jQuery("#donate_form").append("<input type='hidden' name='token' value='" + token + "' />");
					...
					function stripePay(e) {
						e.preventDefault();
						var valid = jQuery("#donate_form").valid();
						if(valid){
							valid = cardValidation();
						}

						if(valid == true) {
							jQuery(".donate_btn_submit").hide();
							jQuery( "#donate_form #loader" ).css("display", "inline-block");
							Stripe.createToken({
				...
				jQuery(document).ready(function() {
							...
							jQuery("#donate_form").validate({
									...
									messages: {
											...
											exp_month: "Please select card expiration month",
											exp_year: "Please select card expiration year",
